...
a single step by taking action $a_{n+1}$.  Furthermore, the agent could learn that
...
\begin{algorithmic}[1]
	\Function{EV}{Episode[] $episodes$, Observation $o$}
		...
		\For{$a$ in $\alpha$}
			...
			\State $nextRuleSet =$ \Call{GetRuleSet}{$nextSeq$}
			\If{$nextRuleSet$ is empty} \hspace{35pt} // base case
				\State $Sum = $ \Call{GetHeuristic}{~} \label{line:heuristic}
			\EndIf
			\For{$rule$ in $nextRuleSet$}
...
\begin{enumerate}
	...
	%\item $nextSeq$ is unique. This implies the agent is in a novel scenario and the current sequence can provide no data to guide the agent. TODO: Update to say that no rules apply, b/c expert knowledge could imply rules in this situation. Also need to update the policy function below.
...
\begin{algorithmic}[1]
	\Function{Policy}{Observation $o$}
		...
		\If{There are no rules with cause sequence $seq, o*$ or $seq$ contains the episode $G$}
		...
		\Else
			\State $prevSeq = seq$
		\EndIf
        \State \Return \Call{GetBestMove}{$prevSeq$, $o$}
	\EndFunction
...
Being in an even state, ARO will be provided an observation of
$0$. Immediately, ARO will form the new rule $\rightarrow 0: 4/7$
because ARO has never seen an even state before. This probability is
chosen because ARO has seen an even state $100\%$ of the time it was
not in an odd state, and based on the rule $\rightarrow 1: 3/7$, ARO
should not be in an odd state $4/7$ths of the time. At this point, ARO
has seen $1$ state and $0$ goals, so $p = \frac{1}{2}$ and our
heuristic is $2$ moves to the goal. (Recall that we add one to the
number of goals when computing $p$.) ARO will compute an expected
value of $2$ for moves $a$ and $b$ since it has no rules to predict
the outcome of either action. In case of a tie, ARO will arbitrarily
pick the earlier move in the alphabet, in this case $a$. Before it
returns, ARO will also update $prevSeq$ to $[0a]$. ARO will then
...
%Those sequences are $[]$ and $[0a]$. For the later sequence, no rule set is found, so the heuristic of $3$ is returned for an arbitrary best move of $a$. For the former sequence, a rule set consisting solely of the new rule $0a \rightarrow 0: 100\%$ is found. However, following this rule recursively in the expected value calculation leads to the same calculation as the expected value of the later sequence, so the expected value of taking move $a$ is $4$. Note that is overlap is common - in our implementation the result is often cached. Comparatively, there is no rule set from making the move $b$, so the heuristic of $3$ is used. Since this is better, $b$ is found to be the best move. In this kind of tie, ARO will prefer the move recommended by the shorter sequence, so it will perform action $b$. ARO then transitions to state $5$.
